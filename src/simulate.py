...
def current_records(schedule: pd.DataFrame) -> dict[int, list[int]]:
    ...
    completed = schedule[status == STATUS_FINAL]
...
def simulate_season_once(
    ...
    """
    Run one simulated timeline of the remaining season. Team strength (Elo
    ratings) is held fixed for the simulation — only home advantage and the
    random game-to-game bounce vary. Returns a DataFrame sorted by wins desc:
    ...
    status = pd.to_numeric(schedule[COL_STATUS], errors="coerce")
    remaining = schedule[status != STATUS_FINAL]
...
if __name__ == "__main__":
    ...
    if schedule is None:
        ...
        final = simulate_season_once(schedule, ratings)
